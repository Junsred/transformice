...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("Transformice", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("Transformice", null, 800, 600);
		#end
		#else
		create(null);
		#end
	...
	public static function create(config):Void
	{
		...
		app.meta["file"] = "Transformice";
		app.meta["name"] = "Transoformice";
		app.meta["packageName"] = "org.openfl.transformice";
		...
		#elseif !air
		app.window.context.attributes.background = 6976661;
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("Transformice")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess Transformice.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"Transformice\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"Transformice\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends Transformice {}
#else
class DocumentClass
{
